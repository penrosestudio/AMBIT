		...
		$.ajax({
			...
			success: function(tiddlers) {
				tiddlers = $.grep(tiddlers, function(t, i) {
					return t.bag!==tiddler.fields['server.bag'];
				});
		...
		var $place = $(place)
			...
			.parent().click(function() {
				var popup = Popup.create(this),
					...
					bag,
					space,
					diffURL = "diff?rev1=bags/"+tiddler.fields['server.bag']+"/"+tiddler.title+"/"+tiddler.fields['server.page.revision']+"&rev2=bags/%0/"+tiddler.title+"/%1&format=horizontal";
				if(popup) {
					...
					$.each(tiddlers, function(i, t) {
						...
						$ul.append('<li><a href="'+diffURL.format(bag, t.revision)+'" target="_blank">'+space+'</li>');
					});
				}
				Popup.show();
				return false;
